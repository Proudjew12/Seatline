...
export function QuoteSummary({ draft, exporting, onExport }: Props) {
  ...
  const totalsOverflow = draft.lines.length > 0 && monthlyValid && annualValid && !totals.valid;
  ...
  return (
    <footer className={styles.summary}>
      <div className={styles.metrics}>
        <div><span>{t("Monthly payments")}</span><output dir="ltr" aria-label={t("Monthly payments")}>{monthlyValid && !totalsOverflow ? formatMoney(totals.monthlyCents) : "—"}</output></div>
        <div><span>{t("Yearly payments")}</span><output dir="ltr" aria-label={t("Yearly payments")}>{annualValid && !totalsOverflow ? formatMoney(totals.annualUpfrontCents) : "—"}</output></div>
        <div><span>{t("Due at start")}</span><output dir="ltr" aria-label={t("Due at start")}>{monthlyValid && annualValid && !totalsOverflow ? formatMoney(totals.dueNowCents) : "—"}</output></div>
      </div>
      ...
      <div className={styles.caption}>
        <span>{t("12-month estimate")} <output dir="ltr" aria-label={t("12-month estimate")}>{monthlyValid && annualValid && !totalsOverflow ? formatMoney(totals.yearEstimateCents) : "—"}</output></span>
      </div>
