...
const LICENSE_LINE_HEIGHT = 4.2;
const PRODUCT_LINE_HEIGHT = 3.8;
...
function rowHeight(layout: QuotePdfLayout, line: QuoteLine): number {
  return Math.max(16, 6 + layout.wrap(cleanPdfField(line.licenseName), 64, 9, "bold").length * LICENSE_LINE_HEIGHT
    + layout.wrap(cleanPdfField(line.productName), 64, 8).length * PRODUCT_LINE_HEIGHT);
}
...
function writeRow(layout: QuotePdfLayout, line: QuoteLine): void {
  ...
  const top = layout.y;
  names.forEach((name, index) => layout.text(name, 22, top + 5 + index * LICENSE_LINE_HEIGHT, 64, 9, "bold"));
  products.forEach((product, index) =>
    layout.text(product, 22, top + 5.5 + names.length * LICENSE_LINE_HEIGHT + index * PRODUCT_LINE_HEIGHT, 64, 8, "normal", PDF_COLORS.muted));
  const [commitment, payment] = BILLING_LABELS[line.billing];
  layout.text(layout.t(commitment), 92, top + 5, 27, 7);
  layout.text(layout.t(payment), 92, top + 10, 27, 6.8, "normal", PDF_COLORS.muted);
  layout.right(String(Number(line.quantity)), 123, top + 5, 9, 8.5);
  layout.right(formatMoney(calculated.customerUnitCents), 135, top + 5, 21, 8.5);
  layout.right(formatMoney(calculated.subtotalCents), 159, top + 5, 29, 8.5, "bold");
  const period = line.billing === "annual-upfront" ? "/ year" : "/ month";
  layout.right(layout.t(period), 135, top + 10, 21, 7, "normal", PDF_COLORS.muted);
  layout.right(layout.t(period), 159, top + 10, 29, 7, "normal", PDF_COLORS.muted);
  layout.y += height;
  layout.rule();
}
